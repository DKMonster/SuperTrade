...
public class getClosePositions {
        ...
        try {
            ...
            if (!statusListener.hasError()) {
                ...
                if (tableManager.getStatus() == O2GTableManagerStatus.TABLES_LOADED) {
                    ...
                    O2GClosedTradesTable closedTradeTable = (O2GClosedTradesTable)tableManager.getTable(O2GTableType.CLOSED_TRADES);
                    O2GOffersTable offersTable = (O2GOffersTable)tableManager.getTable(O2GTableType.OFFERS);
 
                    // Get row level information
                    Data = new Map[closedTradeTable.size()];
                    for (int i = 0; i < closedTradeTable.size(); i++) {
                        HashMap<String, Object> responseData = new HashMap<String, Object>();
                        O2GClosedTradeTableRow closedTrade = closedTradeTable.getRow(i);
                        responseData.put("TradeID" , closedTrade.getTradeID());
                        responseData.put("AccountID" , closedTrade.getAccountID());
                        O2GTableIterator offersIterator = new O2GTableIterator();
                        O2GOfferTableRow offer = offersTable.getNextRowByColumnValue("OfferID", closedTrade.getOfferID(), offersIterator);
                        responseData.put("Instrument" , offer.getInstrument());
                        responseData.put("Amount" , closedTrade.getAmount());
                        responseData.put("BuySell" , closedTrade.getBuySell());
                        responseData.put("OpenRate" , closedTrade.getOpenRate());
                        responseData.put("CloseRate" , closedTrade.getCloseRate());
                        responseData.put("PL" , closedTrade.getPL());
                        responseData.put("GrossPL" , closedTrade.getGrossPL());
                        responseData.put("Commission" , closedTrade.getCommission());
                        responseData.put("RolloverInterest" , closedTrade.getRolloverInterest());
                        responseData.put("OpenTime" , closedTrade.getOpenTime().getTime());
                        responseData.put("CloseTime" , closedTrade.getCloseTime().getTime());
                        Data[i] = responseData;
